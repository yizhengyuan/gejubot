...
async function analyzePosition() {
  ...
  try {
    const quickResp = await fetch("/api/analyze", {
      method: "POST",
    ...
    if (!quickResp.ok) {
      throw new Error(quickData.error || "analyze failed");
    }
    ...
    const refineResp = await fetch("/api/analyze", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
    ...
    if (!refineResp.ok) {
      throw new Error(refineData.error || "analyze refine failed");
    }
    if (requestSeq !== analyzeRequestSeq) {
      return;
    }
